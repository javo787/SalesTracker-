# Дорожная карта внедрения рекламы и партнерства (Ads & Sponsorship Roadmap)

Этот документ описывает стратегию монетизации через рекламу и прямые партнерства для приложения SavdoApp, учитывая баланс между доходностью и пользовательским опытом.
...
### В. Прямая реклама и Спонсорство (Direct Ads / Sponsorship)
Для аудитории предпринимателей (банки Alif, Eskhata, лизинговые компании) реализована система управления прямыми кампаниями.

- **Приоритет**: Прямая реклама (Спонсор) имеет высший приоритет над системной рекламой (Yandex).
- **Формат**: Нативные баннеры с глубокой интеграцией (например, кнопка «Получить кредит для бизнеса» в разделе отчетов).
- **Управление**: Гибкое включение/выключение через удаленную конфигурацию (Remote Config).

### Г. Места размещения (Ad Placements)
1.  **Banner (Баннеры)**:
    - Внизу экрана "Отчеты".
...
- [x] Создание сервиса `src/services/adService.ts` для централизованного управления логикой (проверка 7-дневного периода, таймер 30 минут).
...
- [x] Добавление в `AppContext` флага `isPremium`.
- [ ] Подготовка к интеграции `expo-in-app-purchases` или `react-native-purchases` (RevenueCat) для разового платежа.

### Этап 5: Прямое спонсорство (New)
- [ ] Разработка системы `SponsorshipService` для загрузки кастомных баннеров.
- [ ] Создание компонента `DirectBanner` для отображения акций локальных банков (Alif, Eskhata).
- [ ] Реализация аналитики кликов по прямым предложениям.
