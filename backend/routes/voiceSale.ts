...
router.post('/', authMiddleware, requireShop, (req, res, next) => {
  ...
  try {
    ...
    // Каталог не подмешиваем для 'expense' — там product_name это статья
    // расходов ("аренда", "свет"), а не товар со склада, список товаров
    // там будет только мешать модели.
    const catalogHint = context === 'expense' ? '' : await buildCatalogHint(shopId);
